...
Commands live in `command.history.json` in the platform's own app-data folder, beside the
[clipboard history](clipboard-history.md) and for the same reason — the lines you have
typed are about this machine — as a plain JSON array of strings so the file can be read
and edited by hand. Each is written as it is recorded rather than
